$(document).ready(function() {
    ...
    $("#invoiceNo").autocomplete({
        source: function(request, response) {
            $.ajax({
                url: '../salesinvoice/loadpackagejson',
                dataType: "json",
                data: {
                    q: request.term
                },
        ...
        select: function(event, ui) {
            ...
            $.ajax({
                type: "POST",
                url: "../salesinvoice/getPackageDataByPackageNo",
...
//remove row from table
    ...
    $("#btnSave").click(function() {
        ...
        var r = confirm('Are you sure, Do you want to save '+partType+' ?');
                    if (r === true) {

        $("#modelNotifi").html('');
        if(packageName=='' || packageName==0){
            $.notify("Please enter a package name.", "danger");
            return false;
        }else if(desc.length > 0) {
            $('#btnSave').attr('disabled', true);
            $.ajax({
                url: "../salesinvoice/savePackages",
                type: "POST",
                data: { action: action, packageName:packageName,InvoiceType:InvoiceType,partInvType:partInvType, date: esdate, estimateNo: estimateNo,invoiceNo: invoiceNo,tempInvoiceNo:tempInvoiceNo, remark: remark, estimateAmount: totalAmount, insCompany: insCompany, cusCode: cusCode, regNo: regNo, sup_no: supNum, jobNo: jobNo, job_type: job_type, estimate_type: estimate_type, net_price: net_priceArr, qty: qtyArr, sell_price: sell_priceArr, is_ins: is_insArr, insurance: insuranceArr, desc: descArr, job_id: job_idArr, job_order: job_orderArr, work_id: work_idArr, timestamp: timestampArr,isVat:isVatArr,isNbt:isNbtArr,nbtRatio:nbtRatioArr,proVat:proVatArr,proNbt:proNbtArr,totalPrice:totalPriceArr,proDiscount:proDiscountArr,disPercent:disPercentArr,discountType:disTypeArr,estPrice:estimatePriceArr,costPrice:costPriceArr,
                ...
                success: function(data) {
                    ...
                    if (fb) {
                        ...
                        genarateInvLink(lastproduct_code,0);
...
$("#btnSaveTemp").click(function() {
        ...
        if(jobNo=='' || jobNo==0){
            ...
            $.ajax({
                ...
                type: "POST",
                data: { action: action, InvoiceType:InvoiceType, date: esdate, estimateNo: estimateNo,invoiceNo: tempInvoiceNo, remark: remark, estimateAmount: totalAmount, insCompany: insCompany, cusCode: cusCode, regNo: regNo, sup_no: supNum, jobNo: jobNo, job_type: job_type, estimate_type: estimate_type, net_price: net_priceArr, qty: qtyArr, sell_price: sell_priceArr, is_ins: is_insArr, insurance: insuranceArr, desc: descArr, job_id: job_idArr, job_order: job_orderArr, work_id: work_idArr, timestamp: timestampArr,isVat:isVatArr,isNbt:isNbtArr,nbtRatio:nbtRatioArr,proVat:proVatArr,proNbt:proNbtArr,totalPrice:totalPriceArr,proDiscount:proDiscountArr,disPercent:disPercentArr,discountType:disTypeArr,estPrice:estimatePriceArr,costPrice:costPriceArr,
                nbtRatioRate: nbtRatioRate,isTotalVat:isTotalVat,isTotalNbt:isTotalNbt,totalNet:totalNet,totalAmount:totalAmount,totalVat:finalVat,totalNbt:finalNbt,total_discount:total_discount},
                success: function(data) {
                    var newdata = JSON.parse(data);
                    var fb = newdata.fb;
                    ...
                    if (fb) {
                        ...
                        loadTempInvoiceData(lastproduct_code);
                        isProVatEnabled=0;
                        SupNumber = 0;
                    } else {
                        $("#lastJob").html('');
    ...
    function loadInvoiceData(invNo) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $("#lblremark").html(resultData.inv_hed.remark);
                ...
                $("#lbltotalNet").html(accounting.formatMoney(resultData.inv_hed.JobNetAmount-resultData.inv_hed.JobAdvance));
                ...
                if(resultData.inv_hed.JobVatAmount>0){
                    $("#rowVat").show();
                }else{
                    $("#rowVat").hide();
                }
...
// load grid data
    function loadEstimateDatatoGrid(resultData) {
        ...
        if (resultData.est_dtl){
            ...
            // totalNetAmount = parseFloat(resultData.est_hed.EstNetAmount);
            total_discount=0;
...
//load invoice data to grid
    function loadInvoiceDatatoGrid(resultData) {
        ...
        if (resultData.inv_dtl!=null || resultData.inv_dtl!=''){
            ...
            packageNo = resultData.inv_hed.JobInvNo;
            // loadInvoiceData(resultData.inv_hed.JobInvNo);
            $("#packageName").val(resultData.inv_hed.JobEstimateNo);
...
$("input[name='isTotalNbt']").on('ifChanged', function(event){
    ...
    function genarateInvLink(est,sup){
        ...
        $("#invLink").attr("href","../../admin/payment/job_payment/"+Base64.encode(est));
    }
