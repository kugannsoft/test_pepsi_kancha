...
//    $("#panel_quarter").hide();
    ...
    function loadPO(po_no){
        $.ajax({
            type: "POST",
            url: "../Customerorder/getCoDataById",
            data: { poNo: po_no },
            success: function(data) {
    ...
    $("#grn_no_cancel").autocomplete({
        source: function(request, response) {
            $.ajax({
                url: '../Customerorder/loadpojsonCancel',
                dataType: "json",
                data: {
                    q: request.term,
                    supcode:supcode
...
//===============save products ==============================
    $("#saveItems").click(function() {
        ...
        var grn_no_cancel = $("#grn_no_cancel").val();
        ...
        var r = confirm("Do you want to save this Customer Order.?");
        if (r == true) {
            if (supcode == '' || supcode == '0') {
                ...
                $.ajax({
                    ...
                    data: {grn_no:poNo,action:action,jobNo :jobNo,invoicenumber: invoicenumber, additional: additional, grnremark: grnremark, product_code: sendProduct_code, serial_no: sendSerial_no, qty: sendQty, unit_price: sendUnit_price,
                        ...
                        grnDate: grnDate, invUser: invUser, total_amount: total_amount, total_discount: total_discount, total_net_amount: totalNetAmount, location: location, supcode: supcode, maxSerialQty: maxSerialQty, serialAutoGen: serialAutoGen,nbtRatioRate: nbtRatioRate,isTotalVat:isTotalVat,isTotalNbt:isTotalNbt,totalVat:finalVat,totalNbt:finalNbt,grn_no_cancel:grn_no_cancel},
                    success: function(data) {
                        var resultData = JSON.parse(data);
                        ...
                        if (feedback != 1) {
                            ...
                            $.notify("PO saved successfully.", "success");
                            $("#tbl_item tbody").html('');
                            loadPO(invNumber);
