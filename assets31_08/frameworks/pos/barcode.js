...
//        }
        ...
        if (itemCode == '' || itemCode == 0) {
            ...
            if (itemCodeArrIndex < 0) {
                ...
                i++;
                $("#tbl_item tbody").append("<tr ri=" + i + " id=" + i + " proCode='" + itemCode + "'  qty='" + qty + "' unit_price='" + sellingPrice + "' caseCost='" + casecost + "' discount_percent='" + discount_precent + "' cPrice='" + costPrice + "' pL='" + priceLevel + "' nonDisTotalNet='" + totalNet2 + "' netAmount='" + totalNet + "' proDiscount='" + product_discount + "' proName='"+prdName+"'>\n\
                <td class='text-center'>" + i + "</td><td class='text-left'>" + itemCode + "</td><td>" + prdName + "</td><td class='qty" + i + "'>" + accounting.formatNumber(qty) + "</td><td class='text-right'>" + accounting.formatNumber(sellingPrice) + "</td><td class='rem" + i + "'><a href='#' class='remove btn btn-xs btn-danger'><i class='fa fa-remove'></i></a></td></tr>");
                clear_gem_data();
                setProductTable();
            } else {
...
var r = confirm("Do you want to generate bracode?");
    if (r == true) {
...
//            return false;
            $.ajax({
                type: "post",
                url: "barcodeGenerate2",
                data: { product_code: sendProduct_code, qty: sendQty, unit_price: sendUnit_price,proName:sendPro_name, location: location, supcode: supcode},
                success: function(data) {
                    var resultData = JSON.parse(data);
                    var feedback = resultData['fb'];
                    var invNumber = resultData['InvNo'];
                    if (feedback != 1) {
                        alert('Barcode generate not successfully');
                        $("#loadBarCode").hide();
                         $("#dwnLink").hide();
                        return false;
                    } else {
