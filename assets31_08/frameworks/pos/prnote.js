...
//    $("#panel_quarter").hide();
    ...
    $("#grn_no").autocomplete({
        source: function(request, response) {
            $.ajax({
                url: '../purchase/loadgrnjson',
                dataType: "json",
                data: {
                    q: request.term,supcode:supcode
                },
                success: function(data) {
                    response($.map(data, function(item) {
                        return {
...
//isSup =  $("[name='suppliercheck']:checked").val();
    ...
    $("input[name='grncheck']").on('ifChanged', function() {
        isGrn = $("input[name='grncheck']:checked").val();
        if (!isGrn) {
            isGrn = 0;
...
//===========discount types===========================
    ...
    $("#sellingPrice").blur(function() {
        ...
        if (costPrice > sellingPrice) {
            $.notify("Selling price can not be less than cost price.", "danger");
            return false;
        }
    });
    ...
    $('#sellingPrice').on('keydown', function(e) {
     ...
     function add_products(){
        ...
        if (itemCode == '' || itemCode == 0) {
            $.notify("Please select a item.", "danger");
            return false;
        } else if (sellingPrice == '' || sellingPrice == 0 || isNaN(sellingPrice) == true) {
            $.notify("Selling price can not be 0.", "danger");
            return false;
        } else if (costPrice == '' || costPrice == 0 || isNaN(costPrice) == true) {
            $.notify("Cost price can not be 0.", "danger");
            return false;
        } else if (qty == '' || qty == 0 || isNaN(qty) == true) {
            $.notify("Please enter a qty.", "danger");
            return false;
            ...
            if (is_serail == 0) {
                ...
                if (serialNo == '' || serialNo == 0) {
                    $.notify("Serial Number can not be empty.", "danger");
                    $("#serialNo").focus();
                    return false;
                }
                else if (((serialNoArrIndex >= 0 && is_serail == 1))) {
                    $.notify("Serial Number already added.", "danger");
                    $("#serialNo").val('');
                    return false;
                } else if (((StockserialNoArrIndex < 0 && is_serail == 1))) {
                    $.notify("Serial Number product not in  stock.", "danger");
                    $("#serialNo").val('');
                    return false;
                }
...
$("#serialNo").autocomplete({
        source: function(request, response) {
            $.ajax({
                url: '../../admin/Transer/loadproductSerial',
                dataType: "json",
                data: {
                    q: request.term,
                    location: loc,
...
//        customer_payment = cash_amount + cheque_amount + credit_amount + return_amount;
        ...
        if (r == true) {
            if ((rowCount - 1) == '0' || (rowCount - 1) == '') {
                ...
                $.notify("Please Select a destination location.", "danger");
                return false;
            } else {
...
//            return false;
                $.ajax({
                    type: "post",
                    url: "savePRN",
                    data: {grn_no:grn_no,sup_code:supcode,invoicenumber: invoicenumber, additional: additional, grnremark: grnremark, product_code: sendProduct_code, serial_no: sendSerial_no, qty: sendQty, unit_price: sendUnit_price,
