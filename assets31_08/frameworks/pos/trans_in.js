...
//         $("#location_from").val('');
    ...
    $("#location_to").change(function() {
        ...
        $("#location_from option").each(function() {
            ...
            if ($(this).val() == selval) {
                $(this).attr('disabled', true);
                if ($("#location_from option:Selected").val() == selval) {
                    $("#location_from").val('');
                }
            }
        });
        ...
        if (r == true) {
            if (paymentNo == '' || paymentNo == 0) {
                ...
                $.ajax({
                    type: "POST",
                    url: "saveSTIn",
                    data: {action: "saveSTIn", paymentNo: paymentNo, remark: remark, payDate: payDate, invUser: invUser, supCode: cusCode, invNo: invNo, Item_codeArr: sendItem_code, location: location,location_from:location_from,location_to:location_to},
                    success: function(data)
                    {
                        var resultData = JSON.parse(data);
