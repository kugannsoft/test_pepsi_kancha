                    ...
                    <div class="box-body table-responsive">
                        <table class="table table-bordered" id="producttbl">
                            ...
                            <?php foreach ($transType as $trns) { ?>
                                <tr>
                                    <td><?php echo $trns->function_id; ?></td>
                                    <td><?php echo $trns->class_name; ?></td>
                                    <td><?php echo $trns->function_name; ?></td>
                                    <td>
                                    <a  class="btn btn-xs btn-primary" href="#" onclick="editp(<?php echo $trns->function_id; ?>)" >Edit</a>
                                    </td>
                                </tr>
                                <?php } ?>
                            </tbody>
...
<script type="text/javascript">
    ...
    function addp() {
        $('.modal-content').load('<?php echo base_url() ?>admin/permission/loadmodal_addclass_function/', function (result) {
            $('#productmodal').modal({show: true,backdrop: 'static', keyboard: false});
        });
    }
    function editp(id) {
        console.log(id);
        $('.modal-content').load('<?php echo base_url() ?>admin/permission/loadmodal_editclass_function/', {id: id}, function (result) {
            $('#productmodal').modal({show: true,backdrop: 'static', keyboard: false});
        });
    }
</script>
