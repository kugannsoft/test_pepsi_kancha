            ...
            <div class="col-md-12">
                <div class="box">
                    ...
                    <b><span class="alert alert-success pull-left" id="lastProduct"></span></b>
                        <button onclick="addp()" class="btn btn-flat btn-primary pull-right">Add Job Card Descriptions</button>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <table class="table table-bordered" id="producttbl">
                            ...
                            <?php foreach ($transType as $trns) { ?>
                                <tr>
                                    ...
                                    <td>
                                    <a  class="btn btn-xs btn-primary" href="#" onclick="editp(<?php echo $trns->JobDescNo; ?>)" >Edit</a>
                                    </td>
                                </tr>
                                <?php } ?>
                            </tbody>
...
<script type="text/javascript">
    ...
    function addp() {
        $('.modal-content').load('<?php echo base_url() ?>admin/master/loadmodal_addjobcard_description/', function (result) {
            $('#productmodal').modal({show: true,backdrop: 'static', keyboard: false});
        });
    }
    function editp(id) {
        console.log(id);
        $('.modal-content').load('<?php echo base_url() ?>admin/master/loadmodal_editjobcard_description/', {id: id}, function (result) {
            $('#productmodal').modal({show: true,backdrop: 'static', keyboard: false});
        });
    }
</script>
