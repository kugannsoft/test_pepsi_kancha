            ...
            <div class="col-md-12">
                <div class="box">
                    ...
                    <b><span class="alert alert-success pull-left" id="lastProduct"></span></b>
                        <button onclick="addp()" class="btn btn-flat btn-primary pull-right">Add Job Type</button>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <div class="box-body table-responsive">
                        <table class="table table-bordered" id="producttbl">
                            ...
                            <?php foreach ($transType as $trns) { ?>
                                <tr>
                                    <td><?php echo $trns->jobtype_id; ?></td>
                                    <td><?php echo $trns->jobtype_code; ?></td>
                                    <td><?php echo $trns->jobtype_name; ?></td>
                                    <td><?php echo $trns->jobtype_order; ?></td>
...
<script type="text/javascript">
    ...
    function addp() {
        $('.modal-content').load('<?php echo base_url() ?>admin/master/loadmodal_addjob_type/', function (result) {
            $('#productmodal').modal({show: true,backdrop: 'static', keyboard: false});
        });
    }
    function editp(id) {
        console.log(id);
        $('.modal-content').load('<?php echo base_url() ?>admin/master/loadmodal_editjob_type/', {id: id}, function (result) {
            $('#productmodal').modal({show: true,backdrop: 'static', keyboard: false});
        });
    }
</script>
