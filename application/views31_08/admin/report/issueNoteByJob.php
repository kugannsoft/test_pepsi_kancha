...
<script>
    ...
    function drawTable(data) {
        ...
        $.each(data.pro, function(key, value) {
            ...
            for (var i = 0; i < value.length; i++) {
                ...
                var gr_cost = 0;
                gr_net += parseFloat(value[i].SalesInvAmount);

                if (i == (value.length - 1)) {
                    $("#saletable").append("<tr style='background-color:#3c8dbc;color:#fff;'><td colspan='4' >Total</td><td class='text-right'><b>" + accounting.formatMoney(gr_net) + "</b></td></tr>");
                    $("#saletable").append("<tr><td colspan='9'>&nbsp;</td></tr>");
                    gr_qty = 0;
                    gr_net = 0;
                    gr_pro = 0;
                }
    ...
    function drawRow(rowData, index) {
        ...
        row.append($("<td>" + rowData[index].SalesDate + "</td>"));
        row.append($("<td><a href='<?php echo base_url() ?>admin/Salesinvoice/view_issue_note/"+Base64.encode(rowData[index].SalesInvNo)+"' >" + rowData[index].SalesInvNo + "</a></td>"));
        row.append($("<td>" + rowData[index].SalesPONumber + "</td>"));
        row.append($("<td><a href='<?php echo base_url() ?>admin/payment/view_customer/" + (rowData[index].CusCode) +"' >" + rowData[index].RespectSign +" "+rowData[index].CusName+ "</a></td>"));
        row.append($("<td class='totalamount' align='right'>" + accounting.formatMoney(rowData[index].SalesNetAmount) + "</td>"));
    }
