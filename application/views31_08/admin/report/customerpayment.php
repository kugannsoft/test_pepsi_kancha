                            ...
                            <form id="filterform">
                                ...
                                <div class="col-md-2">
                                    <select class="form-control" name="newsalesperson" id="newsalesperson">
                                        ...
                                        <option value="<?php echo $salesperson->RepID ?>">
                                            <?php echo $salesperson->RepName ?></option>
                                        <?php } ?>
                                    </select>
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        ...
                        <div id="Report_header">
                            <table
                                style="border-collapse:collapse;width:700px;margin:5px;font-family: Arial, Helvetica, sans-serif; margin-left: 180px;"
                                border="0">
                                <tr style="text-align:center;font-size:35px;font-family: Arial, Helvetica, sans-serif;">
                                    <td colspan="6" style="font-size:25px;font-family: Arial, Helvetica, sans-serif;">
                                        <b> <?php echo $company['CompanyName'] ?>
                                            <?php echo $company['CompanyName2'] ?></b></td>
                                </tr>
                                <tr style="text-align:center;font-size:15px;font-family: Arial, Helvetica, sans-serif;">
                                    <td colspan="6">
                                        <b><?php echo $company['AddressLine01'] ?><?php echo $company['AddressLine02'] ?><?php echo $company['AddressLine03'] ?>
                                            &nbsp;&nbsp; <?php echo $company['LanLineNo'] ?>,
                                            <?php echo $company['Fax'] ?> <?php echo $company['MobileNo'] ?></b></td>
                                </tr>
                            </table>
                        </div>
                    </div>
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (data) {
                ...
                drawTable(res.cp);
                $('#cusName').html(res.cp[0].RespectSign + ' ' + res.cp[0].CusName);
                $('#address1').html(res.cp[0].Address01);
                $('#address2').html(res.cp[0].Address02);
...
function drawRow(rowData) {
    ...
    var bank = '';
    if (rowData.BankName != null) {
        bank = (rowData.BankName);
    } else {
        bank = ''
    }
    row.append($("<td>" + rowData.PayDate + "</td>"));
    row.append($("<td class='cashamount' align='right'>" + (rowData.CusPayNo) + "</td>"));
    row.append($("<td class='creditamount' align='right'>" + (rowData.Mode) + "</td>"));
    row.append($("<td class='profit' align='right'>" + accounting.formatMoney(rowData.PayAmount) + "</td>"));
    row.append($("<td class='ccardamount' align='right'>" + checkNull(rowData.ChequeNo) + "</td>"));
    row.append($("<td class='ccardamount' align='right'>" + checkDate(rowData.ChequeDate) + "</td>"));
    row.append($("<td class='ccardamount' align='right'>" + checkDate(rowData.RecievedDate) + "</td>"));
    row.append($("<td class='ccardamount' align='right'>" + checkNull(rowData.Reference) + "</td>"));
    row.append($("<td class='ccardamount' align='right'>" + checkNull(rowData.BankName) + "</td>"));
}
