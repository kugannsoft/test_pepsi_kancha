...
<script>
    ...
    $('#filterform').submit(function(e) {
        ...
        $.ajax({
            type: 'POST',
            url: "loadjobpaymentbyinvoice_without_testing",
            data: $(this).serialize(),
...
var toatl_discount =0;
    function drawTable(data) {
        ...
        $.each(data.pro, function(key, value) {
            ...
            for (var i = 0; i < value.length; i++) {
                ...
                var gr_cost = 0;
                gr_net += parseFloat(value[i].JobInvPayAmount);
    ...
    function drawRow(rowData, index) {
        ...
        var date = new Date(rowData[index].JobInvDate);
        row.append($("<td>" + formatDate(date) + "</td>"));
        row.append($("<td>" + rowData[index].JobInvPayType + "</td>"));
        row.append($("<td>" + rowData[index].Mode + "</td>"));
        row.append($("<td>" + rowData[index].Reference + "</td>"));
        row.append($("<td class='netamount' align='right'>" + accounting.formatMoney(rowData[index].JobInvPayAmount) + "</td>"));
    }
