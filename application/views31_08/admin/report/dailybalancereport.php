 ...
 $("#route").change(function(){
    ...
    function drawTable(data) {
        ...
        for (var i = 0; i < data.bal.length; i++) {
            ...
            $("#suppayment").html(data.bal[i].SUPPLIER_PAYMENT);  
            $("#endFlaot").html(data.bal[i].END_FLOT);  
            $("#startfloat").html(accounting.formatMoney(data.bal[i].START_FLOT));
            $("#balance").html(accounting.formatMoney(data.bal[i].BALANCE_AMOUNT));
            endBalance=parseFloat(data.bal[i].BALANCE_AMOUNT);
            endFloat=parseFloat(data.bal[i].END_FLOT);
