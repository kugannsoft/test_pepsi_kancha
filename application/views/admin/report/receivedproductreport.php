...
<script>
            ...
            success: function(response) {
                ...
                if (Array.isArray(response) && response.length > 0) {
                    $.each(response, function(index, item) {
                        const soldQty = parseFloat(item.TotalSoldQty || 0);
                        const returnedQty = parseFloat(item.TotalReturnedQty || 0);
                        const balance = soldQty - returnedQty;

                        $('#saletable tbody').append(`
                            <tr>
                                <td>${item.SalesProductCode || 'N/A'}</td>
    ...
    function loadprint() {
        $('.modal-content').load('<?php echo base_url() ?>admin/report/', function () {
    ...
    function printdiv() {
        ...
        $("#printReport").print({
            prepend: "<h3 style='text-align:center'>Daily Return Report " + datebalance + "</h3><hr/>",
            title: 'Daily Return Report ' + datebalance
        });
    }
</script>
