...
//        $this->db->where('SalesCustomer', $customer);
    ...
    public function receivedReturnReport1()
    {
        $startdate = $this->input->post('startdate');
        $enddate = $this->input->post('enddate');
        ...
        if (!$startdate || !$enddate || !strtotime($startdate) || !strtotime($enddate)) {
            return $this->output
                ...
                ->set_output(json_encode(['error' => 'Valid start and end dates are required']));
        }
        ...
        salesinvoicedtl.SalesQty,
        received_invoices_items.Quantity
    ');
        $this->db->from('salesinvoicedtl');
        $this->db->join('received_invoices_items', 'salesinvoicedtl.SalesProductCode = received_invoices_items.ProductCode');
