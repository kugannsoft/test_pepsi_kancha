...
class Salesinvoice_model extends CI_Model {
    ...
    public function saveSalesInvoice($grnHed,$post,$grnNo,$totalDisPrecent) {        
        ...
        if($advanceAmount>0){
         ...
         if ($total_Advanceamount > $total_Netamount) {

             $paymentNo = $this->get_max_code('Customer Payment');
             $cash_amount = 0;
             $card_amount = 0;
             $cheque_amount = 0;
             ...
             $total_settle = 0;
             $remark = '';
             $invUser =  $post['invUser'];
             $payMode = 'Advance';

             $cpHed = array(
                 'AppNo' => '1', 'CusPayNo' => $paymentNo, 'PaymentType' => $receiptType, 'CusCode' => $cusCode, 'RootNo' => 0, 'Location' => $location, 'PayDate' => $payDate, 'CashPay' => $payAmount,
                 'ChequePay' => $cheque_amount, 'Remark' => $remark, 'CardPay' => $card_amount, 'TotalPayment' => $payAmount, 'AvailableOustanding' => $avail_outstand, 'CancelUser' => 0, 'SystemUser' => $invUser, 'IsCancel' => 0
             );
            ...
            $this->db->insert('customerpaymentdtl', $cpDtl);
             ...
             $this->db->where('PO_No', $customerPonumber );
             $this->db->where_not_in('ProductCode', $product_codeArr);
             $result = $this->db->update('customerorderdtl', $update_rows);
         }
        ...
        if ($customerPonumber != '') {
            $this->db->update('customerorderhed',array('IsComplate'=>1),array('PO_No'=>$customerPonumber));
        }
...
//                $location=$row[''];
 ...
 public function getSalesInvoiceCustomerPaymentDtlbyid($invNo) {
        $this->db->select('customerorderdtl.*,product.*');
        $this->db->from('customerorderdtl');
        $this->db->join('product', 'product.ProductCode = customerorderdtl.ProductCode', 'left');
        $this->db->where('customerorderdtl.PO_No', $invNo);

        $this->db->order_by('customerorderdtl.PO_LineNo','ASC');
        // $this->db->order_by('jobtype.jobtype_order', 'ASC');
        // $this->db->limit(50);
