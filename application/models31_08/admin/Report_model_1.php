...
class Report_model extends CI_Model {
    ...
    public function genreportbyroute($startdate, $enddate, $location = NULL) {
        if (isset($location) && $location != '') {
            ...
            $this->db->where('InvLocation', $location);
    ...
    public function genreportbyproduct($startdate, $enddate, $location = NULL, $product = NULL) {
        ...
        if (isset($location) && $location != '') {
            $this->db->where('invoicehed.InvLocation', $location);
        }
        if (isset($product) && $product != '') {
            $this->db->where('InvProductCode', $product);
        }
        $this->db->group_by('InvProductCode');
        $this->db->limit(50);
        return $this->db->get()->result();
    }
...
//        $this->db->join('subcategory', 'subcategory.SubCategoryCode = product.SubCategoryCode', 'INNER');
        ...
        if ( isset($subcat) && $subcat != '' ) {
            $this->db->where('product.SubCategoryCode', $subcat);
        }
