            ...
            <div class="col-md-12">
                <div class="box">
                    ...
                    <b><span class="alert alert-success pull-left" id="lastProduct"></span></b>
                        <button onclick="addp()" class="btn btn-flat btn-primary pull-right">Add Class</button>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <div class="box-body table-responsive">
                        <table class="table table-bordered" id="producttbl">
                            ...
                            <?php foreach ($transType as $trns) { ?>
                                <tr>
                                    <td><?php echo $trns->class_id; ?></td>
                                    <td><?php echo $trns->class_name; ?></td>
                                    <td>
                                    <a  class="btn btn-xs btn-primary" href="#" onclick="editp(<?php echo $trns->class_id; ?>)" >Edit</a>
                                    </td>
                                </tr>
                                <?php } ?>
                            </tbody>
...
<script type="text/javascript">
    ...
    function addp() {
        $('.modal-content').load('<?php echo base_url() ?>admin/permission/loadmodal_addclass/', function (result) {
            $('#productmodal').modal({show: true,backdrop: 'static', keyboard: false});
        });
    }
    function editp(id) {
        console.log(id);
        $('.modal-content').load('<?php echo base_url() ?>admin/permission/loadmodal_editclass/', {id: id}, function (result) {
            $('#productmodal').modal({show: true,backdrop: 'static', keyboard: false});
        });
    }
</script>
