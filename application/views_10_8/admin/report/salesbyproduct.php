        ...
        $.ajax({
            ...
            success: function(data) {
               ...
               var d= JSON.parse(data);
                ...
                $('#costval').html(accounting.formatMoney(sumcolumn('costvalue')));
                ...
                $('#earning,#earning2').html(accounting.formatMoney(totalErn));
                 $('#ntsale').html(accounting.formatMoney(sumcolumn('netamount') - (cash_bal) - totalExp + totalErn +proDisount-toatl_discount));
                $('#ntprofit').html(accounting.formatMoney(sumcolumn('profit')-totalExp+totalErn+proDisount-toatl_discount));
            }
        });
    });
var toatl_discount =0;
    function drawTable(data) {
        ...
        $.each(data.pro, function(key, value) {
            ...
            for (var i = 0; i < value.length; i++) {
                ...
                if (value[i].InvProductCode == 104634) {
                    gr_pro = 0;
                } else if (value[i].DepCode == '51') {
                    if (value[i].SubCategoryCode == '2777') {
                        gr_pro += parseFloat(value[i].NetAmount) * 6.3 / 100;
                    } else if (value[i].SubCategoryCode == '2778') {
                        gr_pro += parseFloat(value[i].NetAmount) * 6 / 100;
                    } else if (value[i].SubCategoryCode == '2779') {
                        gr_pro += parseFloat(value[i].NetAmount) * 6 / 100;
                    } else if (value[i].SubCategoryCode == '2780') {
                        gr_pro += parseFloat(value[i].NetAmount) * 6.83 / 100;
                    } else if (value[i].SubCategoryCode == '2781') {
                        gr_pro += parseFloat(value[i].NetAmount) * 4.5 / 100;
                    }

                } else if (value[i].DepCode == '37') {
                    if (value[i].SubCategoryCode == '2259') {
                        gr_pro += parseFloat(value[i].NetAmount) * 25 / 100;
                    } else if (value[i].SubCategoryCode == '2260') {
                        gr_pro += parseFloat(value[i].NetAmount) * 100 / 100;
                    } else if (value[i].SubCategoryCode == '2261') {
                        gr_pro += parseFloat(value[i].NetAmount) * 85 / 100;
                    } else if (value[i].SubCategoryCode == '2262') {
                        gr_pro += parseFloat(value[i].NetAmount) * 40 / 100;
                    } else if (value[i].SubCategoryCode == '2263') {
                        gr_pro += parseFloat(value[i].NetAmount) * 100 / 100;
                    }

                } else {
                    gr_pro += parseFloat(value[i].NetAmount - value[i].CostValue);
                }

                if (i == (value.length - 1)) {
                    $("#saletable").append("<tr style='background-color:#3c8dbc;color:#fff;'><td colspan='4' >Total</td><td><b></b></td><td><b></b></td><td class='cos'>" + accounting.formatMoney(gr_qty) + "</td><td><b></b></td><td><b></b></td><td><b></b></td><td><b>" + accounting.formatMoney(gr_net) + "</b></td><td><b>" + accounting.formatMoney(gr_pro) + "</b></td></tr>");
    ...
    function drawRow(rowData, index) {
        ...
        if (rowData[index].DepCode == '51') {
            if (rowData[index].SubCategoryCode == '2777') {
                profit = parseFloat(rowData[index].NetAmount) * 6.3 / 100;
            } else if (rowData[index].SubCategoryCode == '2778') {
                profit = parseFloat(rowData[index].NetAmount) * 6 / 100;
            } else if (rowData[index].SubCategoryCode == '2779') {
                profit = parseFloat(rowData[index].NetAmount) * 6 / 100;
            } else if (rowData[index].SubCategoryCode == '2780') {
                profit = parseFloat(rowData[index].NetAmount) * 6.83 / 100;
            } else if (rowData[index].SubCategoryCode == '2781') {
                profit = parseFloat(rowData[index].NetAmount) * 4.5 / 100;
            }

        } else if (rowData[index].DepCode == '37') {
            if (rowData[index].SubCategoryCode == '2259') {
                profit = parseFloat(rowData[index].NetAmount) * 25 / 100;
            } else if (rowData[index].SubCategoryCode == '2260') {
                profit = parseFloat(rowData[index].NetAmount) * 100 / 100;
            } else if (rowData[index].SubCategoryCode == '2261') {
                profit = parseFloat(rowData[index].NetAmount) * 85 / 100;
            } else if (rowData[index].SubCategoryCode == '2262') {
                profit = parseFloat(rowData[index].NetAmount) * 40 / 100;
            } else if (rowData[index].SubCategoryCode == '2263') {
                profit = parseFloat(rowData[index].NetAmount) * 100 / 100;
            }

        } else if (rowData[index].InvProductCode == 104634) {
            cash_bal += parseFloat(rowData[index].InvUnitPrice);
            profit = 0;
            cost = 0;
        } else {
        ...
        var date = new Date(rowData[index].InvDate);
        row.append($("<td>" + formatDate(date) + "</td>"));
        row.append($("<td>" + rowData[index].InvNo + "</td>"));
        row.append($("<td>" + rowData[index].InvProductCode + "</td>"));
        row.append($("<td>" + rowData[index].AppearName + "<br> "+rowData[index].Serial+"</td>"));
        row.append($("<td class='invcostpr' align='right'>" + accounting.formatMoney(rowData[index].InvCostPrice) + "</td>"));
        row.append($("<td class='invunitpr' align='right'>" + accounting.formatMoney(rowData[index].InvUnitPrice) + "</td>"));
        row.append($("<td class='qty' align='right'>" + accounting.formatMoney(rowData[index].Qty) + "</td>"));
        row.append($("<td class='freeqty' align='right'>" + accounting.formatMoney(rowData[index].FreeQty) + "</td>"));
        row.append($("<td class='costvalue' align='right'>" + accounting.formatMoney(cost) + "</td>"));
