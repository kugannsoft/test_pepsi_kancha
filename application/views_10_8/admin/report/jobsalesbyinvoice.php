                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-3">
                                        <select class="form-control" name="product" id="product">
                                            ...
                                            <?php foreach ($jobtype AS $prd) { ?>
                                                <option value="<?php echo $prd->jobtype_id ?>"><?php echo $prd->jobtype_name; ?></option>
                                            <?php } ?>
                                        </select>
...
var toatl_discount =0;
    function drawTable(data) {
        ...
        $.each(data.pro, function(key, value) {
            ...
            for (var i = 0; i < value.length; i++) {
                ...
                if (i == (value.length - 1)) {
                    $("#saletable").append("<tr style='background-color:#3c8dbc;color:#fff;'><td colspan='4' >Total</td><td><b></b></td><td class='cos'>" + accounting.formatMoney(gr_qty) + "</td><td></td><td></td><td><b></b></td><td><b></b></td><td><b>" + accounting.formatMoney(gr_net) + "</b></td></tr>");
                    $("#saletable").append("<tr><td colspan='15'>&nbsp;</td></tr>");
                    gr_qty = 0;
                    gr_net = 0;
                    gr_pro = 0;
                }
    ...
    function drawRow(rowData, index) {
        ...
        row.append($("<td class='totalamount' align='right'>" + accounting.formatMoney(rowData[index].TotalAmount) + "</td>"));
        row.append($("<td class='disamount' align='right'>" + accounting.formatMoney(parseFloat(rowData[index].DisAmount)) + "</td>"));
        row.append($("<td class='vatamount' align='right'>" + accounting.formatMoney(parseFloat(rowData[index].VatAmount)) + "</td>"));
        row.append($("<td class='nbtamount' align='right'>" + accounting.formatMoney(parseFloat(rowData[index].NbtAmount)) + "</td>"));
