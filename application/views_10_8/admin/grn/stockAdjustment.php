...
<script>
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $("#saletable").on('click', '.edit', function() {
                    ...
                    if (confirm("Are you want to Update Stock ? Please Check Is correct Product Code")){
                        $.ajax({
                            type: "post",
                            url: baseUrl+'/grn/updateStock',
                            data: {id: id, stockChange:stockChange},
    ...
    function drawRow(rowData, index) {
        ...
        row.append($("<td><input type='number' step='any' value='"+ rowData[index].Stock +"' name='stockChange_" + rowData[index].ProductCode + "' id='stockChange_" + rowData[index].ProductCode + "'>" + rowData[index].Stock + "</td>"));
        ...
        row.append($("<td><button class='btn btn-primary edit' id='saveStock' name='saveStock' value='" + rowData[index].ProductCode + "'>Save</button></td>"));
    }
    function printdiv() {
        $("#saletable").print({
