...
class Product_model extends CI_Model {
   ...
   public function update_data($table, $data, $id) {
    ...
    public function loadproductstockbyid($product,$location,$sup)
    {
    ...
    public function loadproductbypcodenew($product, $pl) {
        return $this->db->select('product.*,productcondition.*,productprice.*')->from('product')
                        ->where('product.ProductCode', $product)
                        ->where('productprice.PL_No', $pl)
                        ->join('productcondition', 'productcondition.ProductCode = product.ProductCode')
                        ->join('productprice', 'productprice.ProductCode = product.ProductCode')
    ...
    public function loaddepartment() {
        return $this->db->select('DepCode,Description')->from('department')->order_by('Description','ASC')->get()->result();
    }

    public function loadsubdepartment($dep) {
        return $this->db->select('*')->from('subdepartment')->where('DepCode', $dep)->order_by('Description','ASC')->get()->result();
    }

    public function loadcategory($subdep,$dep) {
        return $this->db->select('*')->from('category')->where('SubDepCode', $subdep)->where('DepCode', $dep)->order_by('Description','ASC')->get()->result();
    }

    public function loadsubcategory($cat,$subdep,$dep) {
        return $this->db->select('*')->from('subcategory')->where('DepCode', $dep)->where('SubDepCode', $subdep)->where('CategoryCode', $cat)->order_by('Description','ASC')->get()->result();
    }
    ...
    public function loadsuppliers() {
        return $this->db->select('SupCode,SupName')->from('supplier')->order_by('SupName','ASC')->get()->result();
    }
