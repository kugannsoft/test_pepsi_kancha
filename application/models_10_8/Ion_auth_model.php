...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, role, id, password, active, last_login,location, first_name')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
                                    ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
	...
	public function users($groups = NULL)
	{
		...
		else
		{
			...
			$this->db->select(array(
			    ...
			    $this->tables['roles'].'.role as user_role',
			    $this->tables['locations'].'.location as user_branch',
			    $this->tables['companies'].'.CompanyName as company_name',
			));
		}
		...
		$this->db->join(
		    $this->tables['roles'],
		    $this->tables['roles'].'.'.$this->join['role'].'='.$this->tables['users'].'.role',
		    'inner'
		);
		...
		$this->db->join(
		    ...
		    $this->tables['locations'].'.'.$this->join['location'].'='.$this->tables['users'].'.location',
		    'inner'
		);
		...
		$this->db->join(
		    ...
		    $this->tables['companies'].'.'.$this->join['company'].'='.$this->tables['users'].'.com_id',
		    'inner'
		);
	...
	public function user($id = NULL)
	{
		...
		// $this->join($this->tables['location']);
	...
	**/
	...
	protected function _prepare_ip($ip_address) {
        ...
        public function getCompanyData($user)
	{
		$query = $this->db->select('*')
                ->from('users')->where('id', $user)
                ->join('company','company.CompanyID = users.com_id')
                ->limit(1)->order_by('id', 'desc')
                ->get()->row_array();return $query;
        }
}
