...
class Users extends Admin_Controller {
	...
	public function index()
	{
        ...
        else
        {
            ...
            if($_SESSION['role']==1){
	        	...
	        	//$this->template->admin_render('admin', $this->data);
	        }
        }
	}
	...
	public function create()
	{
		...
		$this->form_validation->set_rules('location', 'lang:users_location', 'required');
		$this->form_validation->set_rules('role', 'lang:users_role', 'required');
		...
		$locations = $this->db->select()->from('location')->get()->result();
		$roles     = $this->db->select()->from('role')->get()->result();
		$coms      = $this->db->select()->from('company')->get()->result();
		...
		else
		{
			...
			$this->data['com'] = array(
				...
				'id'    => 'com',
                'class' => 'form-control',
				'value' => $this->form_validation->set_value('com'),
			);
			$this->data['phone'] = array(
				'name'  => 'phone',
				'id'    => 'phone',
				'type'  => 'tel',
                'pattern' => '^((\+\d{1,3}(-| )?\(?\d\)?(-| )?\d{1,15})|(\(?\d{2,6}\)?))(-| )?(\d{3,4})(-| )?(\d{4})(( x| ext)\d{1,15}){0,1}$',
                'class' => 'form-control',
	...
	public function edit($id)
	{
		...
		if ($this->ion_auth->is_admin() && $_SESSION['user_id']==$id)
		{
			// redirect('admin/users', 'refresh');
		}
		else
		...
		if (isset($_POST) && ! empty($_POST))
		{
			...
			if ($this->form_validation->run() == TRUE)
			{
                ...
                if($this->ion_auth->update($user->id, $data))
			    {
				    ...
				    if ($this->ion_auth->is_admin() && ($_SESSION['user_id']==1 || $_SESSION['user_id']==3))
					{
						redirect('admin/users', 'refresh');
					}
					else
					{
						redirect('admin/users/profile/'.$_SESSION['user_id'], 'refresh');
					}
			    }
			    else
			    {
		...
		//  user role show only for admin
		if($user->role==1){
			$this->data['role'] = array(
				'name'  => 'role',
				'id'    => 'role',
	            'class' => 'form-control',
				'value' => $this->form_validation->set_value('role', $user->role),
			);
		}else{
			$this->data['role'] = array(
				'name'  => 'role',
		...
		if($user->role==1){
			$this->data['com'] = array(
				...
				'id'    => 'com',
	            'class' => 'form-control',
				'value' => $this->form_validation->set_value('com', $user->com_id),
			);
		}else{
			$this->data['com'] = array(
				'name'  => 'com',
