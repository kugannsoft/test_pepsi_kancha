...
//==============load products========================
    $("#itemCode").autocomplete({
        source: function(request, response) {
            $.ajax({
                url: baseUrl+'/Salesinvoice/loadreturnproductjson',
                dataType: "json",
                data: {
                    q: request.term,
...
//                    alert(resultData.serial);
                    if (resultData) {
                        ...
                        if(resultData.product){
                            ...
                            //  console.log("stock", stockavalibal);
                        ...
                        if(resultData.pro_stock){
                            console.log(resultData.pro_stock);
                            $("#proStock").html(resultData.pro_stock);
                        }else{
                            $("#proStock").html(0);
                        }
...
//=========Add products===============================
    ...
    function add_products() {
        ...
        if (itemCode == '' || itemCode == 0) {
            ...
            return false;
        }  else if ((qty === '' || qty == 0 || isNaN(qty)) && (freeQty === '' || freeQty == 0 || isNaN(freeQty))) {
            $.notify("Both Qty & Free Qty can not be 0.", "warning");
            return false;
        }
...
// edit grid
    $("#tbl_item tbody").on('click', '.edit', function () {
        ...
        var proname = $row.attr('proName');
        var proCode = $row.attr('proCode');
        var isZero = $row.attr('isZero');
        var qty = parseFloat($row.attr('qty'));
        var returnqty = parseFloat($row.attr('qty'));
        var freeqty = parseFloat($row.attr('fQ'));
        var selprice = parseFloat($row.attr('unit_price'));
        var costprice = parseFloat($row.attr('cPrice'));
        var netprice = parseFloat($row.attr('netAmount'));
        var uc = $row.attr('uc');
        var upc = parseFloat($row.attr('upc'));
        var isSerial = $row.attr('isSerial');
        var serialNo = $row.attr('serialNo');
        var salesPerson = $row.attr('salesPerson');
        var warranty = $row.attr('warranty');
        var pricelevel = $row.attr('pL');
        var totalPrice = parseFloat($row.attr('nonDisTotalNet'));
        var isvat = $row.attr('isvat');
        var isnbt = $row.attr('isnbt');
        var nbtratio = $row.attr('nbtRatio');
        var proVat = parseFloat($row.attr('proVat'));
        var proNbt = parseFloat($row.attr('proNbt'));
        var proDiscount = parseFloat($row.attr('proDiscount'));
        var disPrecent = parseFloat($row.attr('discount_percent'));
        var returntype = $row.attr('returntype');       // NEW
        var SalesReturnQty = $row.attr('qty');            // NEW
        ...
        if (r === true) {
            ...
            $("#SalesReturnQty").val(SalesReturnQty); // Set return type
            $("#isProNbt").prop('checked', isReturn == '1'); // Check return checkbox
    ...
    function loadCustomerDatabyId(customer) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $("#cusName1").html(+resultData.cus_data.CusName+" "+resultData.cus_data.LastName+"</a>");
    ...
    function setGridTableData(resultData) {
        ...
        $.each(orderDetails, function(index, item) {
            ...
            var row = `<tr serial_batch='0' ri="${index + 1}" id="${index + 1}" proCode="${item.productCode}" uc="${item.unitOrCase}" proname = "${item.productName}"
                  ...
                  <td class='text-right'>${item.SalesReturnQty}</td>
                    <td><i class='glyphicon glyphicon-edit edit btn btn-info btn-xs'></i></td>
