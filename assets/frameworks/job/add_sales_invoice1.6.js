     ...
     $('#newsalesperson').on('change', function() {
         ...
         if (salespersonID != "0") {
             ...
             $.ajax({
                ...
                url: baseUrl + '/customer/findemploeeroute',
                 method: 'POST',
...
//    $("#panel_quarter").hide();
    ...
    $("#advance_payment_no").autocomplete({
        source: function(request, response) {
            $.ajax({
                url: baseUrl+'/salesinvoice/loadadvancepaymentjson',
                dataType: "json",
                data: {
                    q: request.term,
                    cusCode:cusCode,
    ...
    $("#return_payment_no").autocomplete({
        source: function(request, response) {
            $.ajax({
                url: baseUrl+'/salesinvoice/loadreturnpaymentjson',
                dataType: "json",
                data: {
                    q: request.term,
                    cusCode:cusCode,
    ...
    if(customerOderNo!=''){
        ...
        loadSAlesInvoiceCusomerPayment(customerOderNo);
    }
    ...
    $("#po_number").autocomplete({
        source: function(request, response) {
            $.ajax({
                url: baseUrl+'/salesinvoice/loadsalesinvoicecustomerpyamentjson',
                dataType: "json",
                data: {
                    q: request.term
                },
    ...
    function loadSAlesInvoiceCusomerPayment(customerOderNo){
        $.ajax({
            type: "POST",
            url: baseUrl+"/Salesinvoice/getSalesInvoiceCustomerPaymentById",
            data: { saleInvoiceNo: customerOderNo },
            success: function(data) {
                var resultData = JSON.parse(data);

                console.log(resultData);
                loadSalesInvoiceCustomerPaymentDatatoGrid(resultData);
            }
        });
    }

    // load grid data
    function loadSalesInvoiceCustomerPaymentDatatoGrid(resultData) {
        ...
        if (resultData.si_hed){
            ...
            cusCode = resultData.si_hed.SupCode;
            isTotalVat =parseFloat(resultData.si_hed.PO_IsVatTotal);
            isTotalNbt =parseFloat(resultData.si_hed.PO_IsNbtTotal);
            totalNbtRatio =parseFloat(resultData.si_hed.PO_NbtRatioTotal);
            loadTotalVATNBT(isTotalVat,isTotalNbt,totalNbtRatio);
            var invlink = baseUrl+"/Salesinvoice/view_sales_invoice/"+Base64.encode(poNo);
            $("#lastInvoice").html("<a href='"+invlink+"'>Last Invoice - "+poNo+"</a>");
            $("#btnPrint").attr('link',invlink);

            // $("#grn_no").val(poNo);
            $("#customer").val(resultData.si_hed.SupCode);
            // $("#invType").val(resultData.si_hed.SalesInvType);
            // $("#salesorder").val(resultData.si_hed.SalesOrderNo);
            $("#grnDate").val(resultData.si_hed.PO_Date);
            ...
            $("#remark").val(resultData.si_hed.Remark);
            ...
            loadCustomerDatabyId(resultData.si_hed.SupCode);
            // getVehiclesByCustomer(resultData.si_hed.SupCode);
            ...
            for (var i = 0; i < resultData.si_dtl.length; i++) {
                ...
                $("#tbl_item tbody").append("<tr ri=" + i + " id=" + i + " proCode='" + resultData.si_dtl[i].ProductCode + "' uc='" + resultData.si_dtl[i].PO_Type + "' qty='" + resultData.si_dtl[i].PO_Qty + "' vatunit_price='" + resultData.si_dtl[i].PO_UnitPrice + "'  org_unit_price='" + resultData.si_dtl[i].PO_UnitPrice + "' unit_price='" + resultData.si_dtl[i].PO_UnitPrice + "' upc='" + resultData.si_dtl[i].PO_UPC + "' caseCost='" + (resultData.si_dtl[i].PO_UnitCost*resultData.si_dtl[i].PO_Qty) + "' isSerial='" + resultData.si_dtl[i].PO_IsVat + "' serial='" + resultData.si_dtl[i].PO_IsVat + "' discount_percent='" + resultData.si_dtl[i].PO_DisPercentage + "' cPrice='" + resultData.si_dtl[i].PO_UnitCost + "' pL='" + 1 + "' fQ='" + 0 + "' nonDisTotalNet='" + resultData.si_dtl[i].PO_TotalAmount	 + "' netAmount='" + resultData.si_dtl[i].PO_NetAmount + "' proDiscount='" + resultData.si_dtl[i].PO_DisAmount + "' proName='" + resultData.si_dtl[i].PO_ProName  + "'   isvat='"+resultData.si_dtl[i].PO_IsVat+"' isnbt='"+resultData.si_dtl[i].PO_IsNbt+"' nbtRatio='"+resultData.si_dtl[i].PO_NbtRatio+"' proVat='"+resultData.si_dtl[i].PO_VatAmount+"' proNbt='"+resultData.si_dtl[i].PO_NbtAmount+"'  salesPerson='"+resultData.si_dtl[i].PO_IsVat+"' >" +
                    "<td class='text-center'>" + (i+1) + "</td>" +
                    "<td class='text-left'>" + resultData.si_dtl[i].ProductCode + "</td>" +
                    "<td>" + resultData.si_dtl[i].PO_ProName + "</td>" +
                    "<td>" + resultData.si_dtl[i].PO_Type + "</td>" +
                    "<td class='qty" + i + "'>" + accounting.formatNumber(resultData.si_dtl[i].PO_Qty) + "</td>" +
                    "<td class='text-right'>" + accounting.formatNumber(resultData.si_dtl[i].PO_UnitPrice) + "</td>" +
                    "<td class='text-center'>" + resultData.si_dtl[i].PO_DisPercentage + "</td>" +
                    "<td class='text-right' >" + accounting.formatMoney(resultData.si_dtl[i].PO_NetAmount) + "</td>" +
                    "<td class='text-right' >" + 0 + "</td>" +
                    "<td></td>" +
                    "<td><i class='glyphicon glyphicon-edit edit btn btn-info btn-xs'></i></td>" +
                    "<td class='rem" + i + "'><a href='#' class='remove btn btn-xs btn-danger'><i class='fa fa-remove'></i></a></td></tr>");
                totalProWiseDiscount += parseFloat(resultData.si_dtl[i].PO_DisAmount);
            }

            // $("#regNo").val((resultData.si_hed.SalesVehicle));
            $("#netgrnamount").html(accounting.formatMoney(resultData.si_hed.PO_NetAmount));
            $("#grndiscount").html(accounting.formatMoney(resultData.si_hed.PO_TDisAmount));
            $("#totalgrn").html(accounting.formatMoney(total_amount));
            $("#totalVat").html(accounting.formatMoney(resultData.si_hed.POVatAmount));
            $("#totalNbt").html(accounting.formatMoney(resultData.si_hed.PONbtAmount));
            $("#shipping").html(accounting.formatMoney(0));
            $("#casAmount").html('Advance Amount For Customer Order : '.concat(accounting.formatMoney(resultData.payment.payTotal)));
            $("#advanceAmountTotal").html('Advance Amount : '.concat(accounting.formatMoney(resultData.advancePayment.payAdvanceTotal)));
            $("#advance_amount").val(resultData.payment.payTotal);
            $("#before_advance_amount").val(resultData.payment.payTotal);
            $("#remark").val(resultData.si_hed.Remark);
            total_discount = parseFloat(resultData.si_hed.PO_TDisAmount);
            total_amount = parseFloat(resultData.si_hed.PO_Amount);
            total_amount2 = parseFloat(resultData.si_hed.PO_Amount);
            totalNetAmount = parseFloat(resultData.si_hed.PO_NetAmount);
    ...
    function loadAdvanceData(pay_no){
        $.ajax({
            type: "POST",
            url: baseUrl+'/salesinvoice/getadvancepaymentbyid',
            data: { payid: pay_no },
            success: function(data) {
                var resultData = JSON.parse(data);
                ...
                if (resultData.advance){
                    ...
                    beforeAdvance = $("#before_advance_amount").val();
                    console.log(parseFloat(beforeAdvance),parseFloat(advance_amount));
                    advance_amount = (parseFloat(advance_amount) + parseFloat(beforeAdvance));
                    $("#advance_amount").val(advance_amount);
                    $("#before_advance_amount").val(advance_amount);
    ...
    function loadSAlesInvoice(po_no){
        $.ajax({
            type: "POST",
            url: baseUrl+"/Salesinvoice/getSalesInvoiceById",
    ...
    function loadSalesOrder(so_no){
        $.ajax({
            type: "POST",
            url: baseUrl+"/Salesinvoice/getSalesOrderById",
            data: { soNo: so_no },
            success: function(data) {
                var resultData = JSON.parse(data);
                ...
                success: function(json) {
                    ...
                    if (resultData) {
                        ...
                        if(resultData.product){
                            ...
                            loadProModal(resultData.product.Prd_Description, resultData.product.ProductCode, resultData.product.ProductPrice, resultData.product.Prd_CostPrice, resultData.product.SerialNo, resultData.product.IsSerial, resultData.product.IsFreeIssue, resultData.product.IsOpenPrice, resultData.product.IsMultiPrice, resultData.product.Prd_UPC, resultData.product.WarrantyPeriod, resultData.product.IsRawMaterial,resultData.product.UOM_Name, resultData.product.ProductVatPrice,resultData.product.DiscountLimit);
                        }
...
//isSup =  $("[name='suppliercheck']:checked").val();
    ...
    // $("input[name='isreturncheck']").on('ifChanged', function() {
    //     isReturn = $("input[name='isreturncheck']:checked").val();
    ...
    $("input[name='isreturncheck']").on('change', function() {
      
    const isReturn = $(this).is(':checked') ? 1 : 0;
...
//                    alert(resultData.serial);
                    ...
                    $("#proStock").html('');
                    $("#priceStock").html('');
                    // $("#prodiscountlimit").html('');
                    // if(resultData.product.DiscountLimit){
                    //     $("#prodiscountlimit").html(resultData.product.DiscountLimit);
                    // }
...
//load model
    function loadProModal(mname, mcode, msellPrice, mcostPrice, mserial, misSerial, misFree, isOP, isMP, upc, waranty, isautoSerial,upm,vatSell,discountLimit) {
//        clearProModal();
        $("#productName").html('');
        $("#qty").focus();
...
//        $("#mLProCode").html(mcode);
        ...
        $("#upc").val(upc);
        $("#upm").html(upm);
         //$("#prodiscountlimit").val(discountLimit);
...
//=========Add products===============================
    ...
    function add_products() {
        ...
        //var isreturncheck = $("#isreturncheck").val();
        let isreturncheck = $('#isreturncheck').is(':checked') ? 1 : 0;
        ...
        //discountLimit = parseFloat(discountLimit) || 0;
         //var prodiscountlimit = $("#prodiscountlimit").val();
        ;
        ...
        if (itemCode == '' || itemCode == 0) {
            ...
            if (is_serail == 0) {
                if ((itemCodesellArrIndex < 1)) {

                    let isReturnText = isreturncheck == 1 ? 'Return' : 'Sale';
                        let returnType = parseInt(returntype);
                        let returnTypeText = '';
                        switch (returnType) {
                            case 1:
                                ...
                                break;
                            case 2:
                                returnTypeText = 'Damage';
                                break;
                            case 3:
                                returnTypeText = 'Expired';
                                break;
                            default:
                                returnTypeText = '';
                    ...
                    // if(discount_precent>prodiscountlimit){
                    //      $.notify("Discount Amount Morethan Limit.", "warning");
                    //         return false;
                    // }
                    ...
                    $("#tbl_item tbody").append("<tr serial_batch='0'  ri=" + i + " id=" + i + " isZero='"+isSellZero+"' proCode='" + itemCode + "' uc='" + unit + 
                        "' qty='" + qty + "' unit_price='" + sellingPrice + "'  vatunit_price='" + vatSellingPrice + "'  org_unit_price='" + orgSellingPrice + 
                        "' upc='" + upc + "' caseCost='" + casecost + "' isSerial='" + is_serail +
                         "' serial='" + serialNo + "' discount_percent='" + discount_precent + "' cPrice='" + costPrice + 
                         "' pL='" + priceLevel + "' fQ='" + freeQty + 
                        "' nonDisTotalNet='" + totalNet2 + "' netAmount='" + totalNet + "' proDiscount='" + product_discount + 
                        "' proName='" + prdName + "'  isvat='"+isNewVat+"' isnbt='"+isNewNbt+"' nbtRatio='"+newNbtRatio+
                        "' proVat='"+proVat+"' proNbt='"+proNbt+"'  salesPerson='"+salesperson+ "'  is_return='"+isreturncheck+ "'  return_type='"+returntype+
                        "'  isbatchSerial='0' warranty='"+warranty+"'>" +
                        ...
                        // "<td>" + serialNo + "</td>" +
                        "<td  class='text-right'>" + isReturnText + "</td>" +
                        "<td  class='text-right'>" + returnTypeText + "</td>" +
                        "<td  class='text-right' style='display:none'>" + isreturncheck + "</td>" +
                        "<td  class='text-right' style='display:none'>" + returnType + "</td>" +
                         "<td  class='text-right' style='display:none'>" + accounting.formatNumber(priceLevel) + "</td>" +
                        "<td><i class='glyphicon glyphicon-edit edit btn btn-info btn-xs'></i></td>" +
                ...
                if (serialNo == '' || serialNo == 0) {
                    ...
                    $("#tbl_item tbody").append("<tr serial_batch='"+serialBatch+"' ri=" + i + " id=" + i + "  isZero='"+isSellZero+
                        "'  proCode='" + itemCode + "' uc='" + unit + "' qty='" + qty + "' unit_price='" + sellingPrice + 
                        "'  vatunit_price='" + vatSellingPrice + "'   org_unit_price='" + orgSellingPrice + "'  upc='" + upc + 
                        "' caseCost='" + casecost + "' isSerial='" + is_serail + "' serial='" + serialNo + 
                        "' discount_percent='" + discount_precent + "' cPrice='" + costPrice + "' pL='" + priceLevel + 
                        "' fQ='" + freeQty + "' nonDisTotalNet='" + totalNet2 + "' netAmount='" + totalNet + "' proDiscount='" + product_discount + 
                        "' proName='" + prdName + "'  isvat='"+isNewVat+"' isnbt='"+isNewNbt+"' nbtRatio='"+newNbtRatio+
                        "' proVat='"+proVat+"' proNbt='"+proNbt+"' salesPerson='"+salesperson+ "'  is_return='"+isreturncheck+ "'  return_type='"+returntype+"' isbatchSerial='1' warranty='"+warranty+"'>" +
                        "<td class='text-center'>" + i + "</td>" +
                        "<td class='text-left'>" + itemCode + "</td>" +
                        "<td>" + prdName + "</td><td>" + unit + "</td>" +
                        "<td class='qty" + i + "'>" + accounting.formatNumber(qty) + "</td>" +
...
// edit grid
    $("#tbl_item tbody").on('click', '.edit', function() {
        ...
        var isReturn = $(this).parent().parent().attr('is_return');
        ...
        if (r === true) {
            ...
            $.ajax({
                type: "post",
                url: baseUrl + "/Product/getProductByIdforSTO",
                data: { proCode: proCode, prlevel: pricelevel, location: 1,price:selprice },
                success: function(json) {
                    var resultData = JSON.parse(json);

                    if (resultData && resultData.productstock) {
                        $("#proStock").html('/ Available Stock = ' + resultData.productstock.Stock);
                        stockavalibal = resultData.productstock.Stock;
                    } else {
                        $("#proStock").html('/ Available Stock = 0');
                    }
                    if(resultData && resultData.price_stock){
                        $("#priceStock").html(resultData.price_stock.Stock);
                    }else{
                        $("#priceStock").html('0');
                    }
                ...
                error: function() {
                    $.notify("Error fetching stock info.", "warning");
                    $("#productStock").html('/ Available Stock = 0');
                    $("#proStock").html('0');
                }
            });
            $("#prdName").val(proname);
            ...
            $("#isreturn").val(isReturn);
             if (isReturn == "1" || isReturn == 1) {
                $('#isreturncheck').prop('checked', true);
            } else {
...
//===============save products ==============================
    $("#saveItems").click(function() {
        ...
        var is_return = new Array();
        var return_type = new Array();
        ...
        var refferNo = $('#refferNo').val();
        var mchange =  $("#mchange1").val();
        ...
        $('#tbl_item tbody tr').each(function(rowIndex, element) {
            ...
            is_return.push($(this).attr('is_return'));
            return_type.push($(this).attr('return_type'));
        });
        ...
        if (r == true) {
            if (cusCode == '' || cusCode == '0') {
                ...
                return false;
            }  else if(totalNetAmount>(cashAmount+cardAmount+chequeAmount+advance_amount+bank_amount+creditAmount+return_amount)){
                ...
                $.ajax({
                    ...
                    data: {route:route,remark:remark,com_amount:com_amount, compayto:compayto,receiver_name:receiver_name,receiver_nic:receiver_nic, refferNo:refferNo, regNo:registerNo,cusCode:cusCode,grn_no:poNo,po_number:po_number,insCompany:insCompany,shippingLabel:shippingLabel,shipping:shipping,
                        newsalesperson:newsalesperson,action:action,salesorder: salesorder, invType:invType, product_code: sendProduct_code, serial_no: sendSerial_no, qty: sendQty, unit_price: sendUnit_price,org_unit_price:sendOrgUnit_price,
                        ...
                        ccAmount: ccAmountArr, ccRef: ccRefArr, ccType: ccTypeArr, ccName: ccNameArr,chequeNo:chequeNo,bank: bank, chequeReference: chequeReference,
                         chequeRecivedDate: chequeReciveDate, chequeDate: chequeDate, mchange: mchange, warrantytype:warrantytypeArr,return_type:return_typeArr,is_return:is_returnArr},
                    success: function(data) {
                        var resultData = JSON.parse(data);
                        ...
                        if (feedback != 1) {
                            ...
                            var invlink = baseUrl+"/Salesinvoice/view_sales_invoice/"+Base64.encode(invNumber);
    ...
    function cal_total(total, discount, extra, downPay, downPayInt, qurPayInt, totalInt, totalExtra) {
        ...
        var coPayment = $('#advance_amount').val();
        ...
        if (coPayment != 0) {
            ...
            if (total > coPayment) {
                ...
                $("#mchange").html(accounting.formatMoney(coPayment-finalAmount));
    ...
    function clear_gem_data() {
        ...
        $("input[name=isreturncheck][value='1']").prop('checked', false);
    ...
    $("#cart-pay-button").click(function(){
        ...
        var before_advance_amount = $("#before_advance_amount").val();
        console.log('asasasas',parseFloat(total_amount),parseFloat(before_advance_amount));
        if (parseFloat(total_amount) < parseFloat(before_advance_amount)) {
...
//     });
    ...
    function getVehiclesByCustomer(cus){
        if(cus!=''){
            $('#regNo').html('');
            $.ajax({
                url: baseUrl+'/job/loadvehiclesjson',
                dataType: "json",
                data: {
                    cusCode: cus
                },
...
// $('#appoDate').datepicker().datepicker("setDate", new Date());
    ...
    function addPayment(pcash, pcredit, pcard,pcheque,pcusType,padvance,pbank,preturn) {
        ...
        $("#mareturn").html(accounting.formatMoney(preturn));
            ...
            success:function(data){
                ...
                if (customerCode) {
                    ...
                    setGridTableData(data);
                     ...
                     $("#hnewsalesperson").html(data.orderHed.RepName);
                     $("#hroute").html(data.orderHed.name);
                    $("#newsalesperson").val(data.orderHed.RepId);
                     $("#route").val(data.orderHed.id);
        ...
        $.each(orderDetails, function(index, item) {
            ...
            var row = `<tr serial_batch='0' ri="${index + 1}" id="${index + 1}" 
                ...
                pL="${item.PriceLevel}" fQ="${item.SalesFreeQty} total_discount="${total_discount}">
                    ...
                    <td class='text-right'>${isReturnText}</td>
                    <td class='text-right'>${returnTypeText}</td>
                    <td class='text-right' style='display: none;'>${item.IsReturn}</td>
                    <td class='text-right' style='display: none;'>${item.ReturnType}</td>
                    <td class='text-right' style='display: none;'>${item.ReturnType}</td>
                    <td class='text-right' style='display: none;'>${item.PriceLevel}</td>
                    <td><i class='glyphicon glyphicon-edit edit btn btn-info btn-xs'></i></td>
